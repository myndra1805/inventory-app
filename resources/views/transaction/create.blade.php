<x-app-layout>
  ...
  <div class="card w-100 shadow border-0">
    <div class="card-body">
      <h3 class="ms-2 text-primary">Add Transaction</h3>
      <form action="/transactions" method="post">
          ...
          <div class="row">
            <div class="col-12 col-lg-6">
              <div class="my-2">
                ...
                <select class="form-select @error('status') is-invalid @enderror" name="status" id="status"
                  ...
                  <option @if (old('status')=='1' ) selected @endif value="1">Incoming Transaction</option>
                  <option @if (old('status')=='0' ) selected @endif value="0">Outgoing Transaction</option>
                </select>
                @error('status')
              ...
              <div class="my-2 @if(old('status')!='1') d-none @endif" id="container-supplier">
                <label for="supplier" class="form-label">Supplier</label>
                <select class="form-select @error('supplier') is-invalid @enderror" name="supplier" id="supplier">
                  <option selected></option>
                  @foreach ($suppliers as $supplier)
                  <option value="{{$supplier->id}}" @if(old('supplier')==$supplier->id) selected
                    @endif>{{$supplier->name}}
                  </option>
                  @endforeach
                </select>
            ...
            <div class="my-2 col-12 col-md-6">
              ...
              <select multiple class="d-none form-select @error('products') is-invalid @enderror" name="products[]"
                ...
                <option value="{{$product->id}}" @if (in_array($product->id, old('products') ? old('products') : []))
              ...
              <ul class="list-group" id="list-product">
                ...
                <li class="list-group-item d-flex align-items-center justify-content-between" id="product-{{$product}}">
                  <p class="m-0">
                  ...
                  <i class="mdi mdi-delete text-danger" data-index="{{$product}}" style="cursor: pointer"
        ...
        <select multiple class="d-none" name="amounts[]" id="amounts">
          @if (old('amounts'))
          @foreach (old('amounts') as $i => $amount)
          <option value="{{$amount}}" selected id="amount-{{old('products')[$i]}}"></option>
          @endforeach
          @endif
        </select>
      </form>
